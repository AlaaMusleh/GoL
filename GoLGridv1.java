...
public class GoLGrid implements Grid {
  ...
  private int numAliveNeighbours(int col, int row) {
    ...
    for (int di = -1; di <= 1; di++) {
      for (int dj = -1; dj <= 1; dj++) {
        if (di == 0 && dj == 0) {
          continue; // the cell itself doens't count.
        }
  ...
  /**
   * Strategy: generate the new grid, copy status from the old one, and replace
   * the reference.
   */
