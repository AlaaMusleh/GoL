...
public class Shell {
  ...
  /**
   * The list of coordinates that need to be marked as "alive" The data structure.
   * is supposed to be Shape ->list of coordaintes.
   */
  private static HashMap<Shell.Shape, ArrayList<Integer[]>> liveCoordinates = 
      new HashMap<Shell.Shape, ArrayList<Integer[]>>() {
    {
  ...
  private static HashMap<Shell.Shape, Integer[]> sizeConstraints = 
      new HashMap<Shell.Shape, Integer[]>() {
    {
  ...
  private static Integer[] getStartCoordinateOfShape(Shell.Shape shape) {
    // prepare the "CORNER" of the shape. which is the upper-left corner of the.
    // shape.
    ...
    /**
     * Reason to start the shape at w / 2 - s / 2, where w is the size of the board,
     * s is the size of the shape, is... |***x***|*** <-- the board, | is the edge
     * of the shape, x is the center of the board and (hopefully) the shape.
     * |------w-------| 
     * |--w/2--| 
     *    |s/2| 
     * |---| 
  ...
  private static final HashMap<String, Integer> paramCount = 
  ...
  // givem an abbreviation or complete command, return the 'capitalized'.
  ...
  private static boolean isAddressValid(int col, int row) {
    ...
    return col >= 0 && col < grid.getColumns() 
  ...
  private static boolean parseCommand(String command) {
    ...
    if (fullCommand == null) {
      ...
      System.out.println("Error! Invalid Input typ H for help");
    ...
    try {
      ...
      System.err.println("Error! Invalid number given.");
      return true;
    ...
    if (grid == null && !(Arrays.asList("QUIT,NEW,HELP".split(","))
        .indexOf(fullCommand) > -1)) {
      System.err.println("Error! Game is not initialized.");
      return true;
    }
    ...
    switch (fullCommand) {
      ...
      case "ALIVE":
        if (!isAddressValid(params[0], params[1])) {
          System.err.println("Error! address out of range");
          return true;
        }
      ...
      case "HELP":
        System.out.println("Welcome to the GameOfLife:"
                  + "\n" + "type new (x y) to create a new grid."
                  + "\n" + "help to print this message."
                  + "\n" + "alive (x y) to set the cell alive"
                  + "\n" + "dead (x y) to set the cell dead"
                  + "\n" + "clear to clear the field"
                  + "\n" + "proint to show the field"
                  + "\n" + "resize (x y) to give the field a new size"
                  + "\n" + "quit to exit");
        break;
      ...
      default : fullCommand = "invaled command.";
        break;
    }
  ...
  /**
   * Requirements: - Clear is called before anything. - Patern is centered. - At
   * least those written on the draft must be implemented.
   */
  private static void shape(String shapeName) {
    ...
    for (Integer[] coordinates : shapeAliveLocation) {
      grid.setAlive(coordinates[0] + startCoordinates[0],
          coordinates[1] + startCoordinates[1], true);
    }
  ...
  public static void main(String[] args) throws IOException {
    InputStreamReader ireader = new InputStreamReader(System.in,"UTF-8");
